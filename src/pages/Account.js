...
function Ticket({purchasedTicket, number, user, setUser}) {

  const [, setError] = useState(false);
  const [, setLoading] = useState(null);
  const [ticketSubmitted, setTicketSubmitted] = useState(purchasedTicket.submitted);
  ...
  <li className="w-full border-[3px] border-black bg-concert-yellowish h-fit md:h-56 flex ">
    ...
    </li>) : (<li className="w-full border-[3px] border-black bg-concert-yellowish h-fit md:h-56 flex ">
      <div
        className={`h-full w-10 bg-${purchasedTicket.bgColor} border-r-[2px] border-black hidden md:block`}
      ></div>
      <div className="h-full p-4">
